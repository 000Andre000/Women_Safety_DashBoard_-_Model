...
const TripPlanner: React.FC = () => {
  ...
  const getTimeCategory = (timeString: string): 'night' | 'earlyMorning' | 'day' | 'evening' => {
  if (!timeString) return 'day'; // Default to day if no time specified
  
  const [hours, minutes] = timeString.split(':').map(Number);
  const decimalTime = hours + minutes / 60;

  // Time category definitions
  if (decimalTime >= 21 || decimalTime < 5) return 'night';       // 9PM - 5AM
  if (decimalTime >= 5 && decimalTime < 8) return 'earlyMorning'; // 5AM - 8AM
  if (decimalTime >= 8 && decimalTime < 18) return 'day';         // 8AM - 6PM
  return 'evening';                                              // 6PM - 9PM
};

const getTimeAdjustedRadius = (zone: any, timeCategory: string): number => {
  // Get the multiplier for the current time category
  const multiplier = zone.timeModifiers?.[timeCategory] || 1;
  
  // Calculate adjusted radius with minimum of original and max of 500m
...
const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
  ...
  <input
    ...
    onChange={(e) => {
      ...
      // You can also set the time category here if needed
      const category = getTimeCategory(e.target.value);
      console.log('Time category:', category);
      // Or store it in state if you need it elsewhere
    }}
    className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
  />
  {/* Optional: Display the time category */}
  {travelTime && (
    <p className="text-sm text-gray-500">
      Time category: {getTimeCategory(travelTime)}
    </p>
  )}
</div>
